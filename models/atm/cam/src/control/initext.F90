...
!
   ...
   ! Initialize constants related to ice
   CALL init_constants ()

   ! Initialize allocatable arrays for MC-KPP
   CALL initialize_mckpp
   
   ! Initialize MC-KPP namelist (currently contained in a separate file, 3D_ocn.nml)
   CALL mckpp_initialize_namelist

   ! Initialize MC-KPP ocean profiles and boundary conditions
   IF (masterproc) WRITE(6,*) 'INITEXT: Calling MCKPP_INITIALIZE_FIELDS'
   CALL mckpp_initialize_fields
   IF (masterproc) WRITE(6,*) 'INITEXT: Returned from MCKPP_INITIALIZE_FIELDS'

   ! Initialize MC-KPP netCDF output files
   IF (masterproc) WRITE(6,*) 'INITEXT: Calling MCKPP_INITIALIZE_OUTPUT'
   CALL mckpp_initialize_output
   IF (masterproc) WRITE(6,*) 'INITEXT: Returned from MCKPP_INITIALIZE_OUTPUT'

   ! Perform initial coupling output of SST and ice to the atmosphere
   IF (masterproc) WRITE(6,*) 'INITEXT: Calling MCKPP_COUPLING_CAM3_OUTPUT'
   CALL mckpp_coupling_cam3_output(srfflx_parm2d_ocn)
   IF (masterproc) WRITE(6,*) 'INITEXT: Returned from MCKPP_COUPLING_CAM3_OUTPUT'   
