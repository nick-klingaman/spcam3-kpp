...
!$OMP PARALLEL DO PRIVATE (C,NCOL,tid)
   do c=begchunk, endchunk      
      tid = omp_get_thread_num()
    !  WRITE(6,*) 'NPK: Thread number ',tid,' executing chunk ',c, &
    !       'min = ',begchunk,' max = ',endchunk
...
!
   ...
   CALL mckpp_coupling_cam3_step(surface_state2d,srfflx_parm2d_ocn)
   !IF (masterproc) STOP
